...
def create_parser():
	...
	parser.add_argument('--save_dir', type=str, default='./checkpoints_2020', help='Models are saved here.')
	parser.add_argument('--load_dir', type=str, default='./checkpoints', help='The directory of the pretrained model.')
	"""important"""
	parser.add_argument('--dataset_dir', type=str,
						default='/data/su_xiaofeng/shrec_data/shrec2020_full_dataset',
...
def train(opt):
	...
	train_dataset = SHREC2020(mode='train', base_dir=opt.dataset_dir, aug_choice=4)
	...
	eval_dataset = SHREC2020(mode='val', base_dir=opt.dataset_dir, aug_choice=0)
	...
	early_stopping = EarlyStopping(save_path=opt.save_dir, patience=6)
	...
	his_df.to_csv('./training_loss/{}_training.csv'.format(datetime.now().strftime("%Y-%m-%d-%H:%M:%S")))

	val_df['total_loss'] = val_loss_list
	val_df['loss_conf'] = val_loss_conf_list
	val_df['loss_cls'] = val_loss_cls_list
	val_df.to_csv('./training_loss/{}_evaluate.csv'.format(datetime.now().strftime("%Y-%m-%d-%H:%M:%S")))
