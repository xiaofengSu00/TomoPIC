...
class SHREC2021(Dataset):
	def __init__(self, mode='train',
				 base_dir='/stroage_data/su_xiaofeng/shrec_data/shrec_2021',
				 detect_size=48,
				 padding=8, aug_choice=0):
		...
		for i in self.data_range:
			...
			with mrcfile.open(os.path.join(self.base_dir, 'model_%d/reconstruction.mrc' % i),
				...
				data = rec_vol.data[166:-166, :, :]
				data = normalize(data)
				self.reconstruction_volume.append(data)
			self.location.append(
				pd.read_csv(os.path.join(self.base_dir, 'model_%d/particle_locations.txt' % i), header=None,
							sep=' '))
...
if __name__ == '__main__':
	train_dataset = SHREC2021(mode='val', base_dir='/storage_data/su_xiaofeng/shrec_data/shrec_2021')
