...
def tom_deconv_tomo(vol_file, out_file, angpix, voltage, cs, defocus, snrfalloff, deconvstrength, highpassnyquist,
					phaseflipped, phaseshift, ncpu=8):
	...
	deconv = np.real(
		scipy.fft.ifftn(scipy.fft.fftn(vol, overwrite_x=True, workers=ncpu) * ramp, overwrite_x=True, workers=ncpu))
		...
		for i in range(self._N[0]):
			for j in range(self._N[1]):
				for k in range(self._N[2]):
					...
					new[i * cubesize:(i + 1) * cubesize, j * cubesize:(j + 1) * cubesize,
					k * cubesize:(k + 1) * cubesize] \
						= one_chunk_data[start:end, start:end, start:end]

		return new[0:self._sp[0], 0:self._sp[1], 0:self._sp[2]]


def deconv_one(tomo, out_tomo, voltage=300.0, cs=2.7, defocus=1.0, pixel_size=1.0, snrfalloff=1.0, deconvstrength=1.0,
			   highpassnyquist=0.02, chunk_size=200, overlap_rate=0.25, ncpu=4):
	import mrcfile
	...
	logging.info(
		'deconv: {}| pixel: {}| defocus: {}| snrfalloff:{}| deconvstrength:{}'.format(tomo, pixel_size, defocus,
																					  snrfalloff, deconvstrength))
	if chunk_size is None:
		tom_deconv_tomo(tomo, out_tomo, pixel_size, voltage, cs, defocus, snrfalloff, deconvstrength, highpassnyquist,
						phaseflipped=False, phaseshift=0, ncpu=ncpu)
	else:
		...
		chunks_deconv_list = []
		with Pool(ncpu) as p:
			partial_func = partial(tom_deconv_tomo, out_file=None, angpix=pixel_size, voltage=voltage, cs=cs,
								   defocus=defocus, snrfalloff=snrfalloff,
								   deconvstrength=deconvstrength, highpassnyquist=highpassnyquist, phaseflipped=False,
								   phaseshift=0, ncpu=1)
...
if __name__ == '__main__':
	...
	# start = time.time()
	# 
	deconv_one(tomo='./TS_0009.rec', 
			out_tomo='./TS_0009_deconv.mrc', 
			voltage=300.0, cs=2.7, 
			defocus=1.0, pixel_size=13.48,snrfalloff=1.0, 
			deconvstrength=1.0,highpassnyquist=0.02,
			chunk_size=200,overlap_rate = 0.25,ncpu=4)
	print('-------')
	# tom_deconv_tomo(args.mrcFile, voltage=args.voltage, cs=args.cs, defocus=args.defocus / 10000.0, angpix=args.pixsize,
	# 				snrfalloff=args.snrfalloff, deconvstrength=args.deconvstrength,
